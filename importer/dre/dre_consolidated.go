...
type DemEarningsConsolidated struct {
	gorm.Model
...
func (DemEarningsConsolidated) TableName() string {
	return "DemResultadoConsolidado"
}

func getDREConsolidatedFromData(data [][]string) ([]*DemEarningsConsolidated, error) {
	total := len(data)
	result := []*DemEarningsConsolidated{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDREConsolidatedRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDREConsolidatedRecord(rec []string) (*DemEarningsConsolidated, error) {
