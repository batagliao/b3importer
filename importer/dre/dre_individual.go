...
type DemEarningsIndividual struct {
	gorm.Model
...
func (DemEarningsIndividual) TableName() string {
	return "DemResultadoIndividual"
}

func getDREIndividualFromData(data [][]string) ([]*DemEarningsIndividual, error) {
	total := len(data)
	result := []*DemEarningsIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDREIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDREIndividualRecord(rec []string) (*DemEarningsIndividual, error) {
