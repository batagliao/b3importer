...
type DemCashFlowIndirectMethodConsolidated struct {
	gorm.Model
...
func (DemCashFlowIndirectMethodConsolidated) TableName() string {
	return "DemFluxoCaixaMetodoIndiretoConsolidado"
}

func getDFCIndirectConsolidatedFromData(data [][]string) ([]*DemCashFlowIndirectMethodConsolidated, error) {
	total := len(data)
	result := []*DemCashFlowIndirectMethodConsolidated{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDFCIndirectConsolidatedRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDFCIndirectConsolidatedRecord(rec []string) (*DemCashFlowIndirectMethodConsolidated, error) {
