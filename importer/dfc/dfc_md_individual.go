...
type DemCashFlowDirectMethodIndividual struct {
	gorm.Model
...
func (DemCashFlowDirectMethodIndividual) TableName() string {
	return "DemFluxoCaixaMetodoDiretoIndividual"
}

func getDFCDirectIndividualFromData(data [][]string) ([]*DemCashFlowDirectMethodIndividual, error) {
	total := len(data)
	result := []*DemCashFlowDirectMethodIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDFCDirectIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDFCDirectIndividualRecord(rec []string) (*DemCashFlowDirectMethodIndividual, error) {
