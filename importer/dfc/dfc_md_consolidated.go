...
type DemCashFlowDirectMethodConsolidated struct {
	gorm.Model
...
func (DemCashFlowDirectMethodConsolidated) TableName() string {
	return "DemFluxoCaixaMetodoDiretoConsolidado"
}

func getDFCDirectConsolidatedFromData(data [][]string) ([]*DemCashFlowDirectMethodConsolidated, error) {
	total := len(data)
	result := []*DemCashFlowDirectMethodConsolidated{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDFCDirectConsolidatedRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDFCDirectConsolidatedRecord(rec []string) (*DemCashFlowDirectMethodConsolidated, error) {
