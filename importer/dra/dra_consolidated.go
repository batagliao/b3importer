...
type DemWideEarningsConsolidated struct {
	gorm.Model
...
func (DemWideEarningsConsolidated) TableName() string {
	return "DemResultadoAbrangenteConsolidado"
}

func getDRAConsolidatedFromData(data [][]string) ([]*DemWideEarningsConsolidated, error) {
	total := len(data)
	result := []*DemWideEarningsConsolidated{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDRAConsolidatedRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDRAConsolidatedRecord(rec []string) (*DemWideEarningsConsolidated, error) {
