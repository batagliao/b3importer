...
type BalancePassiveIndividual struct {
	gorm.Model
...
func (BalancePassiveIndividual) TableName() string {
	return "BalancoPatrimonialPassivoIndividual"
}

func getPassiveIndividualFromData(data [][]string) ([]*BalancePassiveIndividual, error) {
	total := len(data)
	result := []*BalancePassiveIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parsePassiveIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parsePassiveIndividualRecord(rec []string) (*BalancePassiveIndividual, error) {
