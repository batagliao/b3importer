...
type BalanceActiveIndividual struct {
	gorm.Model
...
func (BalanceActiveIndividual) TableName() string {
	return "BalancoPatrimonialAtivoIndividual"
}

func getActiveIndividualFromData(data [][]string) ([]*BalanceActiveIndividual, error) {
	total := len(data)
	result := []*BalanceActiveIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseActiveIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseActiveIndividualRecord(rec []string) (*BalanceActiveIndividual, error) {
