...
type DemAddedValueConsolidated struct {
	gorm.Model
...
func (DemAddedValueConsolidated) TableName() string {
	return "DemValorAdicionadoConsolidado"
}

func getDVAConsolidatedFromData(data [][]string) ([]*DemAddedValueConsolidated, error) {
	total := len(data)
	result := []*DemAddedValueConsolidated{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDVAConsolidatedRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDVAConsolidatedRecord(rec []string) (*DemAddedValueConsolidated, error) {
