...
type DemAddedValueIndividual struct {
	gorm.Model
...
func (DemAddedValueIndividual) TableName() string {
	return "DemValorAdicionadoIndividual"
}

func getDVAIndividualFromData(data [][]string) ([]*DemAddedValueIndividual, error) {
	total := len(data)
	result := []*DemAddedValueIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDVAIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDVAIndividualRecord(rec []string) (*DemAddedValueIndividual, error) {
