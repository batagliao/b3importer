...
type DemEquityChangeIndividual struct {
	gorm.Model
...
func (DemEquityChangeIndividual) TableName() string {
	return "DemMutPatrimonioLiqIndividual"
}

func getDMPLIndividualFromData(data [][]string) ([]*DemEquityChangeIndividual, error) {
	total := len(data)
	result := []*DemEquityChangeIndividual{}
	for i, rec := range data {
		log.Info().Msgf("reading record %d/%d", i+1, total)
		value, err := parseDMPLIndividualRecord(rec)
		if err != nil {
			return nil, err
		}
...
func parseDMPLIndividualRecord(rec []string) (*DemEquityChangeIndividual, error) {
